...
To acquire a preliminary sense of the distribution of the data, I ran a summary.

```{r, echo=TRUE}
summary(df)
```

Next, I plotted histogrames of each of the features to gather a visual sense of each variable's distribution, starting with the feature 'fixed.acidity'.
...
```{r}
print(filter(df, residual.sugar > 50))
```
...
In the first part of my bivariate investigation, I wanted to focus on features that I expected to be highly correlated. I wanted to see if my intuition was correct and look for unexpected relationships between features.
...
cor(df.subset$pH, df.subset$fixed.acidity)
...
print(coefficients(model))
cor(df.subset$pH, df.subset$volatile.acidity)
...
print(coefficients(model))
cor(df.subset$pH, df.subset$citric.acid)
...
|:-------------|:------:|:-----------------:|
|Tartaric acid | 2.89   | -0.40             |
|Citric acid   | 3.13   | -0.17             |
|Acetic acid   | 4.76   | -0.04             |
...
I still noted a great deal of variation. I assume that the variation could be due to the complex chemical composition of the wine. Various buffering agents need to be accounted for, and important chemical species may not have been characterized as part of this study (_e.g._, malic acid, a common component in wine). The weighted measurement of total acid did not have a much stronger correlation with pH as compared with just the tartaric acid measurement. 

```{r echo=TRUE}
cor(df.acid$weighted.total.acid, df.acid$pH)
df.acid.subset <- df.acid %>% 
  filter(weighted.total.acid > quantile(weighted.total.acid, 0.005)) %>% 
  filter(weighted.total.acid < quantile(weighted.total.acid, 0.995))
cor(df.acid.subset$weighted.total.acid, df.acid.subset$pH)
```
...
ggplot(df, aes(x = chlorides, y = density)) + 
  geom_point(alpha = 0.1) + 
  geom_smooth(method = 'lm') +
  geom_smooth(data = subset(df, chlorides < quantile(chlorides, 0.95)), method = 'lm', 
              color = 'red')
```
...
The last solute I investigated with respect to density was sulfates.
...
The correlation appeared weaker than in the case of residual sugar or chlorides.
...
```{r}
ggplot(df, aes(x = quality, y = fixed.acidity, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75) 
```

```{r}
ggplot(df, aes(x = quality, y = volatile.acidity, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = citric.acid, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = residual.sugar, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = chlorides, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = free.sulfur.dioxide, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = total.sulfur.dioxide, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = density, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = pH, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```

```{r}
ggplot(df, aes(x = quality, y = sulphates, group = quality)) + 
  geom_jitter(alpha = 0.1) +
  geom_boxplot(color = 'blue', alpha = 0.75)
```
...
By visual inspection, the most obvious trend seems to be between quality and alcohol content. Otherwise, I could probably convince myself of trends in quality related to chlorides, density, and pH. However, seeing as most of the data fall in the middle of the quality spectrum, proving strong correlations would require a more rigorous approach.
...
```{r}
ggplot(df, aes(x = pH, y = residual.sugar)) +
  geom_point(aes(color = as.factor(quality <= 7), shape = as.factor(quality <= 7)), alpha = 0.2) +
...
df %>% filter(density < 1.02) %>% 
  plot_ly(x = ~residual.sugar, y = ~alcohol, z = ~density,
        marker = list(color = ~density, colorscale = 'Viridis', showscale = TRUE)) %>% 
  add_markers() 
```

Finally, I plotted a correlation matrix to make sure that I covered all the significant relationships variables in the previous section. I also wanted to validate that alcohol was the variable most correlated with quality.

```{r}
# Thanks https://stackoverflow.com/questions/26574054/how-to-change-font-size-of-the-correlation-coefficient-in-corrplot
cex.default <- par('cex')
par(cex = 0.7)
corrplot.mixed(cor(df[,-1]), lower = 'ellipse', upper = 'number', 
               tl.pos = 'lt', diag = 'u', order = 'FPC')
par(cex = cex.default)
```

The above plot suggested a relatively strong correlation between pairs of features that I did not plot in the previous bivariate analysis section. Hence,I decided to plot the graphs below. 
...
**Figure 1** Box and whisker plots of alcohol concentration segmented by quality rating. Individual data points are projected behind the box and whisker plots with jitter to minimize overplotting.

This plot was selected because it showed the strongest relationship between a feature and quality rating.
...
**Figure 2** A three-dimensional plot showing the multivariate relationship between wine density, residual sugar, and alcohol content. 

I selected this plot because it demonstrated a clear relationship between two chemical variables and the density of the wine. 

### Plot Three
```{r}
par(xpd = TRUE) # https://stackoverflow.com/questions/41679136/r-corrplot-crops-bottom-axis-label
corrplot.mixed(
  cor(df[,-1]), 
  lower = 'ellipse', 
  upper = 'number', 
...
**Figure 3** A graphical display of the correlation matrix between features in the dataset. Numeric correlation coefficients are shown in the upper portion of the matrix, while elliptical representations are shown in the bottom half.

Finally, I decided to show a visual representation of the correlation matrix. I believed this would be most useful and efficient preliminary analysis of the data, especially when looking for strong relationships between features.
